...
@section('content')
    <div class="container">
        ...
        <div class="row">
            <div class="col-12">
                <div class="bd-example">
                    @if(count($slider)>0)
                        <div id="carouselExampleCaptions" class="carousel slide" data-animation-delay="{{setting('general.slider_delay')}}ms" data-ride="carousel">
                                ...
                                @foreach($slider as $slide)
                                    @if($loop->first)
                                        <div class="carousel-item active">
                                            <img src="{{ asset($slide->firstMedia('slider')->getUrl()) }}" height="400px" class="d-block w-100" alt="{{ $slide->firstMedia('slider')->filename }}">
                    ...
                    @else
                        <div class="card bg-dark text-black-50" style="height: 500px;">
                            ...
                            <div class="card-img-overlay">
                                ...
                                <p class="card-text">{{__('paragraph')}}</p>
                            </div>
                        </div>
                    @endif
                </div>
        ...
        <div class="d-flex justify-content-center mt-5">
            <div >
                <a href="{{ route('admin.slider.store')}}" class="btn btn-primary " data-toggle="modal" data-target="#sliderModal" data-edit="false">{{ __('Add') }}</a>
                <button class="btn btn-dark " type="button" data-toggle="modal" data-target="#settingModal"><a>{{__('Setting')}}</a></button>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="settingModal">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="sliderModalLabel">{{__('Slider Setting')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form enctype="multipart/form-data" method="post" action="{{ route('admin.settings.update')}}" file="true">
                    @method('PUT')
                    @csrf
                    <div class="row">
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="form-control-1">{{__("Delay Time(Ms)")}}</label>
                        <div class="col-sm-10">
                        <input id="form-control-1" class="form-control"  type="number" name="slider_delay" value="{{setting('general.slider_delay')}}"  >
                        </div>
                    </div>
                    </div>
    ...
    <!-- Modal -->
    <div class="modal fade" id="sliderModal" tabindex="-1" role="dialog" aria-labelledby="addSliderModalLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addSliderModalLabel">{{__('Main slider')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form enctype="multipart/form-data" method="post" action="{{ route('admin.slider.store')}}" file="true">
                        ...
                        @foreach(config('locales') as $locale)
                            <div class="form-group">
                                <label for="header_{{$locale}}">{{ $locale == 'en'? __('Title - English') : __('Title - Arabic') }}</label>
                                <input class="form-control" type="text" name="header_{{$locale}}" value="" id="header_{{$locale}}" >
                            </div>
                        @endforeach
                        @foreach(config('locales') as $locale)
                            <div class="form-group">
                                <label for="paragraph_{{$locale}}">{{ $locale == 'en'? __('Paragraph - English') : __('Paragraph - Arabic') }}</label>
                                <input class="form-control" type="text" name="paragraph_{{$locale}}" value="" id="paragraph_{{$locale}}">
                            </div>
                        @endforeach
                        @foreach(config('locales') as $locale)
                            <div class="form-group">
                                <label for="Direction_{{$locale}}">{{ $locale == 'en'? __('Text Direction - English') : __('Text Direction - Arabic') }}</label>:-&nbsp;&nbsp;
                        ...
                        @foreach(config('locales') as $locale)
                            <div class="form-group">
                                <label for="btn_name_{{$locale}}">{{ $locale == 'en'? __('Button Name - English') : __('Button Name - Arabic') }}</label>
    ...
    <script>
        ...
        function removeSlide(name, url, e) {
            ...
            swal({
                ...
                text: "{{ __('Once deleted, you will not be able to recover this imaginary file!') }}",
                // icon: "warning",
