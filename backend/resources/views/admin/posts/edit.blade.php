                ...
                <form method="POST" action="{{route('admin.posts.update',$post->id)}}" enctype="multipart/form-data" file="true">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">{{__('Updata Post')}}</div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="catBlog">
                                            @foreach($categoriesBlog as $categoryBlog)
                                                <option {{$post->post_cat_id == $categoryBlog->id ? "selected" :''}}    value="{{$categoryBlog->id}}">{{$categoryBlog->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                @foreach(config('locales') as $locale)
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="subject_{{$locale}}" class="form-control" id="subject" placeholder="Enter {{ $locale == 'en'? __('English Name') : __('Arabic Name') }}" value="{{$post->getTranslation('subject',$locale)}}">
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                                ...
                                @foreach(config('locales') as $locale)
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <textarea rows="4" cols="40" maxlength="40" class="form-control" name="sh_desc_{{$locale}}" id="desc" >{{$post->getTranslation('sh_desc',$locale)}}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                                        ...
                                        @foreach(config('locales') as $locale)
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control summernote" id="summernote" name="desc_{{$locale}}">
                                                    {{$post->getTranslation('desc',$locale)}}
                                                </textarea>
                                            </div>
                                        @endforeach
                                       </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        @if($post->hasMedia('post'))
                                            <div class="form-group" id="imgPreview">
                                                <img src="{{asset($post->lastMedia('post')->getUrl())}}" width="100px" height="100px">
                                            </div>
                                        @endif
                                    </div>
