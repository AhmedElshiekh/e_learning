                        ...
                        <form  method="POST" action="{{route('admin.section.setting.update')}}" enctype="multipart/form-data" file="true">
                            ...
                            @foreach(config('locales') as $locale)
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name_{{$locale}}" class="form-control" value="{{$teamSection->getTranslation('name',$locale)}}">
                                </div>
                            @endforeach
                            @foreach(config('locales') as $locale)
                                <div class="form-group">
                                    <label for="description_{{$locale}}">{{ $locale == 'en'? __('English Full Description') : __('Arabic Full Description') }}</label>
                                    <input type="text" name="description_{{$locale}}" class="form-control" value="{{$teamSection->getTranslation('description',$locale)}}">
                                </div>
                            @endforeach

                            <div class="form-group">
                                <label for="backgroungColor">{{__('Section Color')}}</label>
                                <input type="color" name="backgroungColor" value="{{$teamSection->backgroundColor}}">
                            </div>
                            ...
                            <div class="form-row">
                                <div class="col-md-2   text-center">
                                    <div class="form-group" id="imgPreview">
                                        @if($teamSection->hasMedia('team'))
                                            <img src="{{asset($teamSection->firstMedia('team')->getUrl())}}" class="img-fluid"
                                                 alt="">
                                        @else
