    ...
    <div class="row">
        <div class="col-12">
            ...
            @if(isset($partnersBackeground))
                <section style="background-image: url({{ asset($partnersBackeground->firstMedia('partnersBackeground')->getUrl())}});height:500px; background-size: 100% 100%;">
...
{{--                                @dd($partnersLogo)--}}
                              @foreach($partnersLogo as $logo)
                                    <li style="display: inline-block;float: left;width:20%;list-style: none;">
                                        <a href="{{$logo->url}}">
                                            <img class="img-responsive fadeInDown" data-wow-duration="1000ms" data-wow-delay="600ms" src="{{asset($logo->firstMedia('partnersLogo')->getUrl())}}" style="width:100px;height: 100px ">
                                        </a><br>
                                            <a class="btn btn-danger" href="{{route('admin.deleteLogo',$logo->id)}}">delete</a>
                                            <a href="#" class="btn btn-success " onclick='editImage("{{ $logo->url}}","{{ asset($logo->firstMedia('partnersLogo')->getUrl())}}","{{ route('admin.updateLogo', $logo->id) }}", event);' data-toggle="modal" data-target="#editLogoModal" >{{ __('Update') }}</a>
                                    </li>
                              @endforeach
                            </ul>
                        </div>
